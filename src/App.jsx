...
export default function App() {
  ...
  // Remove Transaction & Revert Financial Impact
  const handleRemoveTransaction = (id) => {
    const txToRemove = transactions.find(tx => tx.id === id);
    if (!txToRemove) return;

    setAccounts(accounts.map(acc => 
      acc.id === txToRemove.accountId 
        ? { ...acc, balance: acc.balance + txToRemove.amount } 
        : acc
    ));

    setEnvelopes(envelopes.map(env => 
      env.id === txToRemove.envelopeId 
        ? { ...env, activity: env.activity - txToRemove.amount } 
        : env
    ));
                ...
                <div key={tx.id} className="mobile-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '0.8rem', color: '#666' }}>
                      {tx.date} • {acc ? acc.name : 'N/A'} • {env ? env.name : 'N/A'}
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
